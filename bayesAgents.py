...
def constructBayesNet(gameState):
    ...
    # populating edges and adding a list possible assignments as a value to the variableDomainDict dictionary
    edges.append([X_POS_VAR, GHOST_HOUSE_VAR])
    edges.append([X_POS_VAR, FOOD_HOUSE_VAR])
    variableDomainsDict[X_POS_VAR] = X_POS_VALS #food-left or ghost-left

    edges.append([Y_POS_VAR, GHOST_HOUSE_VAR])
    edges.append([Y_POS_VAR, FOOD_HOUSE_VAR])
    # both-top, both-bottom, left-top, left-bottom
    ...
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            ...
            obsVar = OBS_VAR_TEMPLATE % obsPos
            obsVars += [obsVar]
            variableDomainsDict[obsVar] = OBS_VALS #none, red, or blue

            # Each Observation has a directed arrow FROM FoodHouse or GhostHouse
...
def fillYCPT(bayesNet, gameState):
    ...
    # Y_POS_VALS = both-top, both-bottom, left-top, left-bottom
...
def posToHouseVal(obsPos, gameState):
    # Helper function for fillObsCPT
    # Takes in an observation position along with a gamestate,
    #  and returns one of the variables from HOUSE_VALS that corresponds to the input position
    width, height = gameState.data.layout.width, gameState.data.layout.height
    half_w = width/2
    half_h = height/2

    if obsPos[0] < half_w: # LEFT
        if obsPos[1] > half_h:
            return TOP_LEFT_VAL
        else: return BOTTOM_LEFT_VAL
    else: # RIGHT
        if obsPos[1] > half_h:
            return TOP_RIGHT_VAL
        else: return BOTTOM_RIGHT_VAL
...
def fillObsCPT(bayesNet, gameState):
    ...
    PROB_FOOD_BLUE = 1 - PROB_FOOD_RED
    PROB_GHOST_BLUE = 1 - PROB_GHOST_RED
    ...
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            ...
            for assignment in obsFactor.getAllPossibleAssignmentDicts():
                if assignment[FOOD_HOUSE_VAR] == posToHouseVal(obsPos, gameState):
                        #print "FOOD_HOUSE_VAR", assignment[FOOD_HOUSE_VAR]
                    if assignment[obsVar] == RED_OBS_VAL:
                        obsFactor.setProbability(assignment, PROB_FOOD_RED)
                    elif assignment[obsVar] == BLUE_OBS_VAL:
                        # PROB_FOOD_BLUE = 1 - PROB_FOOD_RED
                        obsFactor.setProbability(assignment, PROB_FOOD_BLUE)
                    else: # No Wall
                        obsFactor.setProbability(assignment, 0)
  
                elif assignment[GHOST_HOUSE_VAR] == posToHouseVal(obsPos, gameState):
                    if assignment[obsVar] == RED_OBS_VAL:
                        obsFactor.setProbability(assignment, PROB_GHOST_RED)
                    elif assignment[obsVar] == BLUE_OBS_VAL:
                        # PROB_GHOST_BLUE = 1 - PROB_FOOD_RED
                        obsFactor.setProbability(assignment, PROB_GHOST_BLUE)
                    else: # No Wall
                        obsFactor.setProbability(assignment, 0)
                else:
                    # The adjacent house center is occupied by neither the ghost house or the food house
                    if assignment[obsVar] == RED_OBS_VAL:
                        obsFactor.setProbability(assignment, 0)
                    elif assignment[obsVar] == BLUE_OBS_VAL:
                        obsFactor.setProbability(assignment, 0)
